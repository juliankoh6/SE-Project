...
class Admin_Dashboard(QtWidgets.QMainWindow):
    ...
    def accept_clinic(self):
        if self.current_clinic:
            clinic_id = self.current_clinic[0]  # Get the clinic ID (assuming it's the first column)
            clinic_email = self.current_clinic[4]  # Get the clinic email (assuming it's the fourth column)
