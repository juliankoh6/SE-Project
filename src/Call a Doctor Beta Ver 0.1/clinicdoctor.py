...
qtCreatorFile = "ui/Clinic_Doctor.ui"
...
class ClinicDoctorApp(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def register_doctor(self):
        ...
        self.cursor.execute("SELECT Doctor_ID, Doctor_Status FROM Doctor WHERE Doctor_Email = ?", (doctor_email,))
        doctor_record = self.cursor.fetchone()
        if doctor_record is not None:
            if doctor_record[1] == 1:  # Doctor_Status is 1
                QMessageBox.critical(self, "Registration Failed", "Email already exists and is in use by a current doctor.")
                return
            else:
                self.cursor.execute("DELETE FROM Doctor WHERE Doctor_ID = ?", (doctor_record[0],))  # Delete the old record
        ...
        try:
            ...
            QMessageBox.information(self, "Email Sent", f"Registration email sent to {doctor_email}.")
        except sqlite3.Error as e:
            QMessageBox.critical(self, "Database Error", str(e))

    def clear_form_fields(self):
        self.DoctorNameInput.clear()
        self.DoctorEmailInput.clear()
        self.DoctorJobInput.clear()
        self.DoctorPasswordInput.clear()
        self.SpecialityListWidget.clearSelection()
