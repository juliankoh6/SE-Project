...
def test_send_request(qtbot):
    ...
    qtbot.wait(2000)
    qtbot.keyClicks(window.Email_line, 'kendeji@gmail.com')
    qtbot.wait(1000)
    qtbot.keyClicks(window.Password_line, 'sadicehappyfire')
    qtbot.wait(1000)

    qtbot.mouseClick(window.LoginpushButton, Qt.LeftButton)
    qtbot.wait(1000)

    qtbot.wait_until(lambda: window.msg_box_name.isVisible())
    qtbot.mouseClick(window.msg_box_name.button(QMessageBox.Ok), Qt.LeftButton)

    qtbot.wait_until(lambda: window.patient_dashboard_window.ui.clinic_listWidget.count() > 0)

    qtbot.wait(1000)
    Clinic_item = window.patient_dashboard_window.ui.clinic_listWidget.item(0)
    text = Clinic_item.text()

    window.patient_dashboard_window.ui.clinic_listWidget.setCurrentItem(Clinic_item)

    qtbot.mouseClick(window.patient_dashboard_window.ui.clinic_listWidget.viewport(), Qt.LeftButton,
                     pos=window.patient_dashboard_window.ui.clinic_listWidget.visualItemRect(Clinic_item).center())
    qtbot.wait(1000)

    qtbot.wait_until(lambda: window.patient_dashboard_window.ui.doctor_list.count() > 0)

    Doctor_item = window.patient_dashboard_window.ui.doctor_list.item(1)
    Doctor_Text = Doctor_item.text()
    qtbot.mouseClick(window.patient_dashboard_window.ui.doctor_list.viewport(), Qt.LeftButton,
                     pos=window.patient_dashboard_window.ui.doctor_list.visualItemRect(Doctor_item).center())
    qtbot.wait(1000)

    qtbot.mouseClick(window.patient_dashboard_window.ui.send_request_button, Qt.LeftButton)
    qtbot.wait(1000)

    clinic_text = window.patient_dashboard_window.ui.request_page.ui.clinicname_entry.text()
    doctor_name = window.patient_dashboard_window.ui.request_page.ui.doctorname_entry.text()

    qtbot.keyClicks(window.patient_dashboard_window.ui.request_page.ui.requestreason_edit, 'AAA')
    qtbot.wait(1000)

    request_reason = window.patient_dashboard_window.ui.request_page.ui.requestreason_edit.toPlainText()

    qtbot.wait(1000)

    qtbot.addWidget(window.patient_dashboard_window.ui.request_page.ui.Request_dateEdit)
    new_datetime = QDateTime(QDate(2024, 7, 16), QTime(14, 30))
    window.patient_dashboard_window.ui.request_page.ui.Request_dateEdit.setDateTime(new_datetime)
    qtbot.wait(1000)

    qtbot.mouseClick(window.patient_dashboard_window.ui.request_page.ui.sendrequest_button, Qt.LeftButton)
    qtbot.wait(1000)
    ...
    conn = sqlite3.connect('call_a_doctor.db')
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM Patient_Request WHERE Patient_ID = ? AND Request_Reason = 'AAA'", (window.patient_dashboard_window.patient_id,))
    result = cursor.fetchone()
    conn.close()

    print('            ')

    print(window.patient_dashboard_window.patient_id)
    print(window.patient_dashboard_window.ui.patient_id)
