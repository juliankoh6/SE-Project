        ...
        def Send_request():
            ...
            if request_reason == "":
                ...
                for items in find_date:
                    ...
                    given_time = datetime.strptime(items[7], "%H:%M:%S")

                    # Calculate 2 hours after the given time
                    future_after_two_hours = given_time + timedelta(hours=2)
                    future_after_two_hours_str = future_after_two_hours.strftime("%H:%M:%S")

                    # Calculate 2 hours before the given time
                    past_before_two_hours = given_time - timedelta(hours=2)
                    past_before_two_hours_str = past_before_two_hours.strftime("%H:%M:%S")


                    print('aaaaaaaaaaaaaa')
                    print(future_after_two_hours_str)
                    print(past_before_two_hours_str)

                    if request_date == items[6]:
                        if (past_before_two_hours_str < request_time < future_after_two_hours_str) and (items[4] != 2):
                            datetime_result = 1
